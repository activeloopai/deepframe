...
core::buffer_ptr extract_video_frames_from_video_at_url_(const std::string& url,
                                                         ...
                                                         AVPixelFormat force_pixel_format = AV_PIX_FMT_NONE)
{
    ...
    if (codecContext->width <= 0 || codecContext->height <= 0) {
        std::cerr << "Invalid video dimensions: " << codecContext->width << "x" << codecContext->height << std::endl;
        avcodec_free_context(&codecContext);
        avformat_close_input(&formatContext);
        return nullptr;
    }
    ...
    auto frames_buffer = core::buffer::create(
        {static_cast<ssize_t>(original_indices.size()), codecContext->height, codecContext->width, 3});
    ...
    auto processFrame = [&](AVFrame* frame) -> bool {
        ...
        bool needs_seek = ((start_frame - current_frame) > 300) && tried_to_seek_to_frame_ == -1;
        ...
        if (current_frame == start_frame) {
            if (indices_it->second != current_frame) {
                std::cerr << "invalid state: current_frame: " << current_frame << " was not found in the indices"
                          << std::endl;
                return true;
            }

            // Check the frame details
            if (frame->width != codecContext->width || frame->height != codecContext->height) {
                std::cerr << "Frame resolution mismatch!" << std::endl;
                return false; // Continue processing next frames
            }

            int64_t offset = indices_it->first * codecContext->height * codecContext->width * 3;
            ...
            sws_scale(swsContext, frame->data, frame->linesize, 0, codecContext->height, chunks, linesize);

            ++indices_it;

            while (indices_it != indexed_list.end() && indices_it->second == current_frame) {
                auto current_offset = indices_it->first * codecContext->height * codecContext->width * 3;
                std::memcpy(frames_buffer->buffer_at_offset(current_offset),
                            chunks[0],
...
std::map<std::string, double, std::less<>> get_video_info(const std::string& url)
{
    ...
    if (avformat_find_stream_info(format_ctx, nullptr) != 0) {
        std::cerr << "Could not find stream info for video file: " << url << std::endl;
    ...
    if (info["num_frames"] == 0) {
        ...
        if (duration > 0 && framerate.num > 0 && framerate.den > 0) {
            info["num_frames"] = static_cast<uint64_t>(av_q2d(framerate) * duration * av_q2d(video_stream->time_base));
        }
    }

    AVRational fps_rational = video_stream->avg_frame_rate;
    info["fps"] = fps_rational.den > 0 ? static_cast<double>(fps_rational.num) / fps_rational.den : 0;
...
core::buffer_ptr extract_video_frames_from_video_at_url(const std::string& url, const std::vector<int64_t>& indices)
{
    return extract_video_frames_from_video_at_url_(url,
                                                   indices,
#if __APPLE__
                                                   AV_PIX_FMT_YUV420P
#else
