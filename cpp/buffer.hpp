...
public:
    buffer_overflow_exception(int64_t size, size_t max_size)
        : exception(std::string("buffer overflow exception, tried to get: " + std::to_string(size) +
...
public:
    ...
    const std::vector<ssize_t>& dims() const
    {
        return dims_;
    }
...
private:
    buffer(const std::vector<ssize_t>& dims)
    {
        ...
        dims_ = dims;
        allocated_size_ = std::accumulate(dims_.begin(), dims_.end(), 1, std::multiplies<ssize_t>()) * sizeof(uint8_t);
