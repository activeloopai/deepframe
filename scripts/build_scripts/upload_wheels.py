#!/usr/bin/env python3

import sys
import os
import boto3

RUN_ID = os.environ.get("RUN_ID")
RUNNER_TYPE = os.environ.get("TYPE")
PYTHON = os.environ.get("PYTHON")

KEY = "indra/deepframe"

if sys.argv[1] == "deepframe":
    W_PATH = f"dist/centos_wheels_{RUN_ID}_{RUNNER_TYPE}_{PYTHON}.zip"
    os.system(f"cd dist; zip centos_wheels_{RUN_ID}_{RUNNER_TYPE}_{PYTHON}.zip *.whl")
else:
    print("wrong preset")
    sys.exit(1)

S3_CONSOLE_URL = f"https://us-east-1.console.aws.amazon.com/s3/object/activeloop-dev-tests?region=us-east-1&bucketType=general&prefix={KEY}/{W_PATH}"

client = boto3.client("s3")

with open(W_PATH, "rb") as file:
    client.upload_fileobj(
        Fileobj=file, Bucket="activeloop-dev-tests", Key=f"{KEY}/{W_PATH}"
    )

with open(os.environ.get("GITHUB_STEP_SUMMARY"), "a", encoding="utf-8") as file:
    file.write(f"|{PYTHON}_{RUNNER_TYPE}|\n")
    file.write("|-|\n")
    file.write(f"|[s3://activeloop-dev-tests/{KEY}/{W_PATH}]({S3_CONSOLE_URL})|\n")
